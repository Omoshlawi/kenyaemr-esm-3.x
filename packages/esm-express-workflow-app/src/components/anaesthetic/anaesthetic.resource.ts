    ...
    try {
      ...
      for (const encounter of data.results) {
        ...
        const amnioticFluidObs = encounter.obs.find(
          (obs: PartographyObservation) => obs.concept.uuid === PARTOGRAPHY_CONCEPTS['amniotic-fluid'],
        );
        const mouldingObs = encounter.obs.find(
          (obs: PartographyObservation) => obs.concept.uuid === PARTOGRAPHY_CONCEPTS['moulding'],
        );
...
function toOpenMrsIsoWithColonOffset(date: Date): string {
  const iso = toOmrsIsoString(date);
...
export async function saveMembraneAmnioticFluidData(
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to save fetal heart rate data');
  }
}
export type { PartographyObservation, PartographyEncounter };
const defaultAnaestheticConfig = configSchema.anaesthetic._default;
const getConceptMappingsForGraphType = (graphType: string): string[] => {
  return (
    defaultAnaestheticConfig.conceptMappings[graphType as keyof typeof defaultAnaestheticConfig.conceptMappings] || []
  );
};

const getFallbackEncounterTypes = (graphType: string): string[] => {
  return (
    defaultAnaestheticConfig.fallbackEncounterMapping[
      graphType as keyof typeof defaultAnaestheticConfig.fallbackEncounterMapping
    ] || defaultAnaestheticConfig.defaultFallbackSequence
  );
};

const getStationDisplay = (conceptUuid: string): string => {
  return (
    defaultAnaestheticConfig.stationDisplayMapping[
      conceptUuid as keyof typeof defaultAnaestheticConfig.stationDisplayMapping
    ] || conceptUuid
  );
};
...
const getStationValue = (conceptUuid: string): number => {
  return (
    defaultAnaestheticConfig.stationValueMapping[
      conceptUuid as keyof typeof defaultAnaestheticConfig.stationValueMapping
    ] ?? 0
  );
};
...
const getProgressEventInfo = (conceptUuid: string): { name: string; unit: string } | null => {
  return (
    defaultAnaestheticConfig.progressEventConceptNames[
      conceptUuid as keyof typeof defaultAnaestheticConfig.progressEventConceptNames
    ] || null
  );
};

const getGraphTypeDisplayName = (graphType: string): string => {
  const config = defaultAnaestheticConfig;
  const mapped = config.graphTypeDisplayNames[graphType as keyof typeof config.graphTypeDisplayNames];
  return mapped || graphType.replace('-', ' ').replace(/\b\w/g, (l) => l.toUpperCase());
};
...
const generateCacheKey = (patientUuid: string, graphType: string): string => {
  const config = defaultAnaestheticConfig;
  return `${config.storage.cacheKeyPrefix}${patientUuid}_${graphType}`;
};

const getDefaultLocationUuid = (): string => {
  const config = defaultAnaestheticConfig;
  return config.defaultLocationUuid;
};

const getDefaultEncounterProviderRole = (): string => {
  const config = defaultAnaestheticConfig;
  return config.defaultEncounterProviderRole;
};
...
async function discoverEncounterTypes(): Promise<{ [key: string]: string }> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      encounterTypes.forEach((et: { name: string; uuid: string; display?: string }) => {
...
async function getEncounterTypeForGraph(graphType: string): Promise<string | null> {
  const predefinedType = PARTOGRAPHY_ENCOUNTER_TYPES[graphType as keyof typeof PARTOGRAPHY_ENCOUNTER_TYPES];
  if (predefinedType) {
  ...
  for (const pattern of defaultAnaestheticConfig.encounterPatterns) {
  ...
  try {
    ...
    if (!finalLocationUuid) {
      try {
        ...
        // Session location not available, will use default
      }
    ...
    const encounterDatetime = toOpenMrsIsoWithColonOffset(new Date(Date.now() - timeConfig.defaultEncounterOffset));
    ...
    return {
      ...
      message:
        t?.('failedToSavePartographyData', 'Failed to save partography data: {{error}}')?.replace(
          '{{error}}',
          error instanceof Error ? error.message : 'Failed to save partography data',
        ) ||
        `Failed to save partography data: ${
          error instanceof Error ? error.message : 'Failed to save partography data'
        }`,
    };
  }
...
function buildObservations(graphType: string, formData: any): any[] {
  const observations = [];
  const timeConfig = getTimeConfig();
  const obsDatetime = toOpenMrsIsoWithColonOffset(new Date(Date.now() - timeConfig.defaultEncounterOffset));
  ...
  const MAINTENANCE_OF_ANAESTHESIA_CONCEPT = '164254AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
  const CONCENTRATION_RATE_CONCEPT = '160632AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
  const MEDICATION_GIVEN_CONCEPT = '164231AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
  const POST_OPERATIVE_POSITION_CONCEPT = '357b7ae8-1326-4c2c-b1c0-7e7260fba2b7';
  const ESTIMATED_BLOOD_LOSS_CONCEPT = '161928AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
  const RESULTS_OF_OPERATION_CONCEPT = 'da2029a1-193f-48df-88d8-45d3258458c1';
  const POST_OPERATIVE_MANAGEMENT_CONCEPT = 'dd22e450-256e-4925-bcd1-ce39454fb46f';
  ...
  switch (graphType) {
    case 'pulse-bp-combined':
      ...
      if (formData.heartRate || formData.pulse) {
        observations.push({
          concept: ANAESTHETIC_HEART_RATE_CONCEPT,
          value: parseFloat(formData.heartRate ?? formData.pulse),
          obsDatetime,
        });
      }
      if (formData.systolic) {
        observations.push({
          concept: ANAESTHETIC_SYSTOLIC_BP_CONCEPT,
          value: parseFloat(formData.systolic),
          obsDatetime,
        });
      } else if (formData.systolicBP) {
        observations.push({
          concept: ANAESTHETIC_SYSTOLIC_BP_CONCEPT,
          value: parseFloat(formData.systolicBP),
          obsDatetime,
        });
      }
      if (formData.diastolic) {
        observations.push({
          concept: ANAESTHETIC_DIASTOLIC_BP_CONCEPT,
          value: parseFloat(formData.diastolic),
          obsDatetime,
        });
      } else if (formData.diastolicBP) {
        observations.push({
          concept: ANAESTHETIC_DIASTOLIC_BP_CONCEPT,
          value: parseFloat(formData.diastolicBP),
          obsDatetime,
        });
      }
      if (formData.spo2) {
        observations.push({
          concept: ANAESTHETIC_SPO2_CONCEPT,
      ...
      if (formData.etco2) {
        observations.push({
          concept: ANAESTHETIC_ETCO2_CONCEPT,
          value: parseFloat(formData.etco2),
          obsDatetime,
        });
      }
      break;
    case 'membrane-amniotic-fluid':
      ...
      if (amnioticFluidValue && amnioticFluidMap[amnioticFluidValue as keyof typeof amnioticFluidMap]) {
        amnioticFluidValue = amnioticFluidMap[amnioticFluidValue as keyof typeof amnioticFluidMap];
      }
      ...
      if (mouldingValue && mouldingValue in mouldingMap) {
        mouldingValue = mouldingMap[mouldingValue as keyof typeof mouldingMap];
      }
      if (mouldingValue) {
        observations.push({
    ...
    case 'fetal-heart-rate':
      ...
      if (formData.hour !== undefined && formData.hour !== '') {
        try {
          ...
          // Skipping invalid hour observation
        }
      }
      ...
      if (formData.time) {
        try {
          ...
          // Skipping invalid time observation
        }
      }
      break;
    ...
    case 'drugs-fluids':
      if (formData.maintenanceAgent) {
        observations.push({
          concept: MAINTENANCE_OF_ANAESTHESIA_CONCEPT,
          value: formData.maintenanceAgent,
          obsDatetime,
        });
      }
      if (formData.concentrationRate) {
        observations.push({
          concept: CONCENTRATION_RATE_CONCEPT,
          value: String(formData.concentrationRate).trim(),
          obsDatetime,
        });
      }
      if (formData.medicationGiven) {
        observations.push({
          concept: MEDICATION_GIVEN_CONCEPT,
          value: formData.medicationGiven,
          obsDatetime,
        });
      }
      if (formData.fluidsGiven) {
        observations.push({
          concept: MEDICATION_GIVEN_CONCEPT,
          value: formData.fluidsGiven,
          obsDatetime,
        });
      }
    ...
    case 'post-operative-summary':
      if (formData.position) {
        observations.push({
          concept: POST_OPERATIVE_POSITION_CONCEPT,
          value: formData.position,
          obsDatetime,
        });
      }
      if (
        formData.estimatedBloodLoss !== undefined &&
        formData.estimatedBloodLoss !== null &&
        formData.estimatedBloodLoss !== ''
      ) {
        observations.push({
          concept: ESTIMATED_BLOOD_LOSS_CONCEPT,
          value: Number(formData.estimatedBloodLoss),
          obsDatetime,
        });
      }
      if (formData.resultsOfOperation) {
        observations.push({
          concept: RESULTS_OF_OPERATION_CONCEPT,
          value: formData.resultsOfOperation,
          obsDatetime,
        });
      }
      if (formData.postOperativeManagement) {
        observations.push({
          concept: POST_OPERATIVE_MANAGEMENT_CONCEPT,
          value: formData.postOperativeManagement,
          obsDatetime,
        });
      }
      break;
    ...
    default:
      if (formData.value || formData.measurementValue) {
        const conceptUuid = PARTOGRAPHY_CONCEPTS[graphType as keyof typeof PARTOGRAPHY_CONCEPTS];
        if (conceptUuid) {
          observations.push({
            concept: conceptUuid,
            value: parseFloat(formData.value || formData.measurementValue),
...
export async function createTestPartographyData(patientUuid: string) {
  ...
  for (const [index, graphType] of testGraphTypes.entries()) {
    const enhancedDataPoints = sampleDataPoints.map((point: { time: string; value: number }) => ({
      ...point,
    ...
    for (const dataPoint of enhancedDataPoints) {
      ...
      try {
        await createPartographyEncounter(patientUuid, graphType, formData);
      } catch (error) {}
    }
  }
}
    ...
    try {
      ...
      for (const encounter of data.results) {
        ...
        const fetalHeartRateObs = encounter.obs.find(
          (obs: PartographyObservation) => obs.concept.uuid === PARTOGRAPHY_CONCEPTS['fetal-heart-rate'],
        );
...
export async function saveFetalHeartRateData(
  ...
  try {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to save fetal heart rate data',
    };
  }
}
