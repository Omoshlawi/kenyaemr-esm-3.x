...
import { DefaultWorkspaceProps, showSnackbar, useConfig, Workspace2 } from '@openmrs/esm-framework';
...
const PatientCaseForm: FC<PatientCaseFormProps> = ({ closeWorkspace, patientUuid }) => {
  ...
  const onSubmit = async (values: FormType) => {
    try {
      ...
      showSnackbar({
        kind: 'error',
        title: t('error', 'Error'),
        subtitle: t('errorAddingCase', 'Error Adding patient case' + (error?.message || JSON.stringify(error))),
        isLowContrast: true,
      });
    }
  };
  ...
  return (
    <Workspace2 title={t('addPatientCaseForm', 'Add Patient Case Form')} hasUnsavedChanges={hasUnsavedChanges}>
