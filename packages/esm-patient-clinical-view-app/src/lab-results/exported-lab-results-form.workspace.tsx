...
import orderStyles from './order-basket.scss';
import MalariaLabResultsComponent from './malaria-lab-results.component';
import { allAdditionalMalariaConceptUuids } from './malaria-lab-results.constants';
...
const ExportedLabResultsForm: React.FC<Workspace2DefinitionProps<LabResultsFormProps, {}, {}>> = ({
  ...
  const { isLoading: isLoadingMalariaConcepts, concepts: malariaAdditionalConcepts } = useOrderConceptsByUuids(
    allAdditionalMalariaConceptUuids,
  );
  const [showEmptyFormErrorNotification, setShowEmptyFormErrorNotification] = useState(false);

  const allConcepts = useMemo(
    () => [...conceptArray, ...malariaAdditionalConcepts],
    [conceptArray, malariaAdditionalConcepts],
  );

  const compositeSchema = useMemo(() => createLabResultsFormCompositeSchema(allConcepts), [allConcepts]);
  ...
  if (isLoadingResultConcepts || isLoadingMalariaConcepts) {
    return (
      <Workspace2
  ...
  const saveLabResults = async (formValues: Record<string, unknown>) => {
    ...
    const obsPayload = createCompositeObservationPayload(allConcepts, order, formValues, 'FINAL');
              ...
              <Stack gap={5}>
                {!isLoading ? (
                  <>
                    ...
                    <MalariaLabResultsComponent
                      formValues={formValues}
                      completeLabResults={completeLabResults}
                      control={control as unknown as Control<Record<string, unknown>>}
                      setValue={setValue}
                    />
                  </>
