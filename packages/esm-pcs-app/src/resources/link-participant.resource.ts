...
/**
 * Express-workflow's visit form props, redeclared rather than imported: the workspace registry is
 * global so the form can be launched by name across the app boundary, but its module cannot be.
 */
...
import { findExistingLocalPatient, searchLocalPatientByIdentifier } from './patient-search.resource';
import { createPatientFromHiePatient } from './patient-create.resource';
import { formatParticipantName } from './pcs.resource';
import { type IdentifierTypeUuids, type PcsParticipant, type PcsSearchSubject } from '../types';
 ...
 *
 * The create is kept separate from the check-in deliberately: express-workflow's own
 * `registerOrLaunchHIEPatient` launches the visit workspace inside its create branch, which
 * would fire before the study data is written.
 */
async function resolveLocalPatient(subject: PcsSearchSubject, uuids: IdentifierTypeUuids) {
  const existing = await findLocalPatientForSubject(subject);
  if (existing) {
    return existing;
  }

  return createPatientFromHiePatient(subject.patient, uuids);
}
...
export async function linkParticipantToPatient({
  ...
  const localPatient = await resolveLocalPatient(subject, uuids);
