...
import { createDependantPatient, createPatientFromHiePatient } from './patient-create.resource';
...
vi.mock('./patient-create.resource', async (importOriginal) => ({
  ...(await importOriginal<typeof import('./patient-create.resource')>()),
  createDependantPatient: vi.fn(),
  createPatientFromHiePatient: vi.fn(),
}));
vi.mock('./patient-search.resource', () => ({ findExistingLocalPatient: vi.fn() }));
vi.mock('./identifiers', async (importOriginal) => ({
  ...(await importOriginal<typeof import('./identifiers')>()),
  openmrsId: 'openmrs-id-type',
  openmrsIdSource: 'openmrs-id-source',
  generateIdentifier: vi.fn(async () => ({ data: { identifier: 'MGH-0001' } })),
}));

const mockOpenmrsFetch = vi.mocked(openmrsFetch);
const mockCreateDependantPatient = vi.mocked(createDependantPatient);
const mockCreatePatientFromHie = vi.mocked(createPatientFromHiePatient);
const mockFindExistingLocalPatient = vi.mocked(findExistingLocalPatient);

const uuids = {
  nationalIdUUID: 'national-id-type',
  shaNumberUUID: 'sha-number-type',
  passportUUID: 'passport-type',
  birthCertificateUUID: 'birth-certificate-type',
  crIdentificationNumberUUID: 'cr-number-type',
...
describe('linkDependantToParticipant', () => {
  it('uses the existing local record and never creates one', async () => {
    ...
    expect(mockCreateDependantPatient).not.toHaveBeenCalled();
  ...
  it('creates the dependant, then stamps, then checks in exactly once', async () => {
    ...
    mockCreateDependantPatient.mockResolvedValue({ uuid: 'new-uuid', identifiers: [] } as any);

    await link();

    expect(mockCreateDependantPatient).toHaveBeenCalledOnce();
    // The dependant and the parent's number reach the creator; the identifier types come from
    // config rather than being resolved inside it.
    expect(mockCreateDependantPatient).toHaveBeenCalledWith(dependant, uuids, '0712345678');
...
describe('createAndLinkFromParticipant', () => {
  ...
  const createAndLink = (subject: PcsParticipant = withContacts) =>
    createAndLinkFromParticipant({
      participant: subject,
      nationalIdUUID: uuids.nationalIdUUID,
      phoneAttributeTypeUUID: uuids.phoneAttributeTypeUUID,
      studyParticipantIdentifierType: STUDY_ID_TYPE,
      pbidsEnrollmentAttributeType: PBIDS_ENROLLMENT_TYPE,
      cardseEnrollmentAttributeType: CARDSE_ENROLLMENT_TYPE,
    });
  ...
  it('carries the phone and national ID across from the PCS contact', async () => {
    ...
    // Asserted by content rather than position: `preferred` is what designates the primary
    // identifier, so the order within the array carries no meaning.
