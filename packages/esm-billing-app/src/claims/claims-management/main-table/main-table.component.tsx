...
const MainTable: React.FC<MainTableProps> = ({ fromDate, toDate, onDateChange }) => {
  ...
  const [pageSize, setPageSize] = useState(CLAIMS_PAGE_SIZE);

  const startIndex = (currentPage - 1) * pageSize;
  const { data, claims, isLoading, error } = useClaimsMetrics(
    fromDate,
    toDate,
  ...
  const totalClaims = data?.metrics?.total_claims ?? 0;
  ...
  const headers = [
    ...
    { key: 'modeOfArrival', header: t('modeOfArrival', 'Mode of arrival') },
  ...
  const tableRows = filteredClaims.map((claim) => {
    ...
    return {
      ...
      modeOfArrival: claim.mode_of_arrival ?? t('walkIn', 'WALK-IN'),
